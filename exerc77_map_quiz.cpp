...
class Question {
  public:
    Question(int questionNumber, string question, int answerIndex)
      : m_questionNumber(questionNumber), m_question(question), m_answerIndex(answerIndex) {}
...
// Custom comparison object that the map will use to sort the keys
...
int main() {
    ...
    while (quizIterator != quiz.end()) {
      ...
      cout << (wasCorrect ? "CORRECT!" : "INCORRECT!")
           << " The correct answer is: " << quizIterator->second[correctAnswer] << endl << endl;
